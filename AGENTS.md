...
5. **改造 DSH（激活动态插件）——插件源码已随部署包位于 `plugins/`（克隆即含），初始化时按本步部署**
   - **先切换到「创造模式」**：动态插件的定义与运行依赖 `cordis_define` / `cordis_run` 工具，仅在创造模式（具备 Cordis 动态插件能力的会话）可用。若当前会话不是创造模式，**先切换再执行本步**。
